...
export const RevPDFUploaderAndViewer = () => {
  const [file, setFile] = useState<string | undefined>(() => {
    const savedFile = sessionStorage.getItem("pdfBlobUrl2");
  ...
  const handleFileUpload = async (file: File) => {
    ...
    setFile(blobUrl);
    sessionStorage.setItem("pdfBlobUrl2", blobUrl);

    try {
      const formData = new FormData();
      formData.append("file", file);

      const response = await fetch(`http://localhost:${API_PORT}/add_reviewee`, {
        method: "POST",
        body: formData,
      });
  ...
  const handleReset = () => {
    setFile(undefined);
    sessionStorage.removeItem("pdfBlobUrl2");
  };
...
export default RevPDFUploaderAndViewer;
